...
__attribute__((always_inline)) inline void subgroup_impl(const T* input, T* output, T* loc, T* loc_twiddles,
  ...
  global2local<level::WORKGROUP, SubgroupSize, pad::DONT_PAD, 0>(it, twiddles, loc_twiddles, N_reals_per_wi * FactorSG);
  ...
  for (std::size_t i = id_of_fft_in_kernel; i < rounded_up_n_ffts; i += n_ffts_in_kernel) {
    ...
    if constexpr (TransposeIn == detail::transpose::TRANSPOSED) {
      ...
      if (it.get_local_linear_id() / 2 < num_batches_in_local_mem) {
        ...
        global2local_transposed<detail::level::WORKGROUP, detail::pad::DO_PAD, bank_groups_per_pad, T>(
      ...
      for (std::size_t sub_batch = id_of_fft_in_sub_batch; sub_batch < rounded_up_sub_batches;
        ...
        if (_working) {
          ...
          local2private_transposed<FactorWI, detail::pad::DO_PAD, bank_groups_per_pad>(
        ...
        if constexpr (SubgroupSize == FactorSG) {
          if (_working) {
            ...
            store_transposed<N_reals_per_wi, detail::pad::DONT_PAD, 0>(priv, output, id_of_wi_in_fft, FactorSG,
                                                                       (i + sub_batch) * n_reals_per_fft);
          }
        } else {
          if (_working) {
            // Store back to local memory only
            private2local_transposed<FactorWI, detail::pad::DO_PAD, bank_groups_per_pad>(
      ...
      if constexpr (SubgroupSize != FactorSG) {
        ...
        local2global_transposed<detail::pad::DO_PAD, bank_groups_per_pad>(
            it, FactorWI * FactorSG, num_batches_in_local_mem, max_num_batches_local_mem, loc, output,
            i * n_reals_per_fft);
      }
      sycl::group_barrier(it.get_group());
    } else {
      ...
      global2local<level::SUBGROUP, SubgroupSize, pad::DO_PAD, bank_groups_per_pad>(
          it, input, loc, n_ffts_worked_on_by_sg * n_reals_per_fft, n_reals_per_fft * (i - id_of_fft_in_sg),
          subgroup_id * n_reals_per_sg);

      sycl::group_barrier(sg);
      if (working) {
        local2private<N_reals_per_wi, pad::DO_PAD, bank_groups_per_pad>(loc, priv, subgroup_local_id, N_reals_per_wi,
                                                                        subgroup_id * n_reals_per_sg);
      }
      sycl::group_barrier(sg);
      ...
      if constexpr (FactorSG == SubgroupSize) {
        ...
        if (working) {
          store_transposed<N_reals_per_wi, pad::DONT_PAD, bank_groups_per_pad>(
              priv, output, id_of_wi_in_fft, FactorSG, i * n_reals_per_sg + id_of_fft_in_sg * n_reals_per_fft);
        }
      } else {
        if (working) {
          store_transposed<N_reals_per_wi, pad::DO_PAD, bank_groups_per_pad>(
              priv, loc, id_of_wi_in_fft, FactorSG, subgroup_id * n_reals_per_sg + id_of_fft_in_sg * n_reals_per_fft);
        }
        sycl::group_barrier(sg);
        local2global<level::SUBGROUP, SubgroupSize, pad::DO_PAD, bank_groups_per_pad>(
