  ...
  for (std::size_t i = global_id; i < roundUpToMultiple(n_transforms, SubgroupSize); i += global_size) {
    ...
    if (working) {
      if constexpr (TransposeIn == detail::transpose::TRANSPOSED) {
        ...
        local2private<N_reals, pad::DO_PAD, bank_groups_per_pad>(loc, priv, subgroup_local_id, N_reals, local_offset);
      }
      ...
      private2local<N_reals, pad::DO_PAD, bank_groups_per_pad>(priv, loc, subgroup_local_id, N_reals, local_offset);
    }
    sycl::group_barrier(sg);
